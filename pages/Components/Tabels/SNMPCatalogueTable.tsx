...
const SNMPCatalogueTable = (props: any) => {
  ...
  //    // return date.toLocaleString() + ', ' + date.toLocaleTimeString(); // Format date and time according to user's locale
  //   };
  //  // const csvData = [Object.keys(selectedRowsData[0])]; // Header
  //  const csvData = [Object.keys(selectedRowsData[0]).map(key => 
  //   (key === 'created_on' || key === 'updated_on') ? key.replace('_', ' ') : key
  // )]; // Header with modified field names
  // selectedRowsData.forEach((row: any) => {
  //   const rowData: any = Object.values(row).map((value: any) => 
  //     (typeof value === 'number' && (row['created_on'] === value || row['updated_on'] === value)) ? formatTimestamp(value) : value.toString()
  //   );
...
const downloadExcel = () => {
  ...
  // Set column widths
  const columnWidths = [{wch: 15}, {wch: 20}, {wch: 20}, {wch: 20}, {wch: 25}, {wch: 25}, {wch: 15}, {wch: 20}, {wch: 15}, {wch: 20}, {wch: 15}];
            ...
            <div className="flex">
              ...
              <div className="m-4 mr-0 ml-2 h-fit">
                ...
                <Button
                  ...
                  size="small"
                >
                  <AddIcon fontSize="small" className="mr-2" /> Add SNMP
                  Catalogue
                </Button>
                {/* <AddIcon
                  className=" dark:text-textColor"
